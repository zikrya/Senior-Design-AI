  ...
  const handleSubmit = async (e, customTopic = null) => {
    ...
    try {
      ...
      const response = await fetch(HTTP, {
        ...
        body: JSON.stringify({ help: help }),
      });

      if (response.ok) {
        const responseText = await response.text();
        setResponses([responseText]);
        // setError(null);
      } else {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
        <div className="w-full md:w-2/3 mx-auto p-5 bg-white rounded-lg shadow-md">
            <div className="text-center mb-6">
                <h2 className="text-3xl font-extrabold text-indigo-500 mb-6">Need help?</h2>
             </div>
